...
def movietypelookup(mykey, searchstring, typestring):
    """Interactions with OMDB API
       mykey = omdb api key
       searchstring = string to search for"""
    try:
        # begin constructing API
        api = f"{OMDBURL}apikey={mykey}&s={searchstring}&type={typestring}"
    ...
    while True:
        ...
        while answer == "":
            ...
            ** Returned data will be written into the local database **
            ...
            2) Search for Movies Containing String, and by Type (movie, series or episode)
            99) Exit""")
        ...
        if answer == "2":
            searchstring = input("Please enter the title of what you are looking for: ")
            typestring = input("Please enter the type you are looking for (movie, series or episode): ")

            if answer == "2":
                resp = movietypelookup(mykey, searchstring, typestring)
                #display the results
                resp = resp.get("Search")
                print(resp)
                #write the results into the database
